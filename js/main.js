/*
FILE NAME: main.js
ASSIGNMENT: Implementation of a Scrabble game with Drag-and-Drop features
Name: Mohammed Kabir
email: Mohammed_Kabir@student.uml.edu
Course: GUI Programming I
Date: 6/23/2022
FILE DESCRIPTION:  The purpuse of this file is to do the calculation and creation of 
the game. It has function for creating the board of the game, reseting the game, shuffling the 
words, counting scores for each players and many more. I have included little description for
each functions on the top of them. 
Project description: Please read the Report.txt file for project description and 
it's implemention.
*/
...
$(function() {
    ...
    var createBag = function() {
        tileBag = [
            { letter: "E", score: 1, count: 12, view: "images/tiles/Scrabble_Tile_E.jpg"},
            { letter: "A", score: 1, count: 9, view: "images/tiles/Scrabble_Tile_A.jpg" },
            { letter: "I", score: 1, count: 9, view: "images/tiles/Scrabble_Tile_I.jpg" },
            { letter: "O", score: 1, count: 8, view: "images/tiles/Scrabble_Tile_O.jpg" },
            { letter: "N", score: 1, count: 6, view: "images/tiles/Scrabble_Tile_N.jpg" },
            { letter: "R", score: 1, count: 6, view: "images/tiles/Scrabble_Tile_R.jpg" },
            { letter: "T", score: 1, count: 6, view: "images/tiles/Scrabble_Tile_T.jpg" },
            { letter: "L", score: 1, count: 4, view: "images/tiles/Scrabble_Tile_L.jpg" },
            { letter: "S", score: 1, count: 4, view: "images/tiles/Scrabble_Tile_S.jpg" },
            { letter: "U", score: 1, count: 4, view: "images/tiles/Scrabble_Tile_U.jpg" },
            { letter: "D", score: 2, count: 4, view: "images/tiles/Scrabble_Tile_D.jpg" },
            { letter: "G", score: 2, count: 3, view: "images/tiles/Scrabble_Tile_G.jpg" },
            { letter: "B", score: 3, count: 2, view: "images/tiles/Scrabble_Tile_B.jpg" },
            { letter: "C", score: 3, count: 2, view: "images/tiles/Scrabble_Tile_C.jpg" },
            { letter: "M", score: 3, count: 2, view: "images/tiles/Scrabble_Tile_M.jpg" },
            { letter: "P", score: 3, count: 2, view: "images/tiles/Scrabble_Tile_P.jpg" },
            { letter: "F", score: 4, count: 2, view: "images/tiles/Scrabble_Tile_F.jpg" },
            { letter: "H", score: 4, count: 2, view: "images/tiles/Scrabble_Tile_H.jpg" },
            { letter: "V", score: 4, count: 2, view: "images/tiles/Scrabble_Tile_V.jpg" },
            { letter: "W", score: 4, count: 2, view: "images/tiles/Scrabble_Tile_W.jpg" },
            { letter: "Y", score: 4, count: 2, view: "images/tiles/Scrabble_Tile_Y.jpg" },
            { letter: "K", score: 5, count: 1, view: "images/tiles/Scrabble_Tile_K.jpg" },
            { letter: "J", score: 8, count: 1, view: "images/tiles/Scrabble_Tile_J.jpg" },
            { letter: "X", score: 8, count: 1, view: "images/tiles/Scrabble_Tile_X.jpg" },
            { letter: "Q", score: 10, count: 1, view: "images/tiles/Scrabble_Tile_Q.jpg" },
            { letter: "Z", score: 10, count: 1, view: "images/tiles/Scrabble_Tile_Z.jpg" },
            { letter: "", score: 0, count: 2, view: "images/tiles/Scrabble_Tile_blank.jpg" }
        ]
    }
    ...
    var createBoard = function() {
        for (i = 0; i < 15; i++) {
            var newRow = $('<div class="row g-row"></div>');
            for (j = 0; j < 15; j++) {
    ...
    var shuffleBag = function() {
        ...
        //     const rndm = Math.floor(Math.random() * tempBag.length);
        //     console.log(rndm);
        //     shuffledBag.push(tempBag[rndm]);
        //     tempBag.splice(rndm, 1);
        // }

        shuffledBag = shuffle(tempBag);
    ...
    var endGame = function() {
        ...
        $('.playerTwoBox').show();
        $('.playerTwoTitle').show();
        $('.playerOneBoxRow').hide();
        $('.playerTwoBoxRow').hide();
    ...
    var isItPlayerOnesTurn = function() {
        if ($('.playerOneBox').css("display") !== 'none') {
            return true;
        } else {
            return false;
        }
    ...
    var loadRack = function(player) {
        ...
        for (i = player.rack.length; i < 7; i++) {
            if (shuffledBag.length > 0) {
                ...
                if (player === players[0]) {
                    ...
                    $('.playerOneBoxRow').append(newTileBox);
                } else {
                    var newTileBox = $('<div class="playerTwoTile tileBox"></div>');
                    $('.playerTwoBoxRow').append(newTileBox);
                }
                $(".tileBox").draggable();
            }
        }
        ...
        if (player === players[0]) {
            for (j = 0; j < $('.playerOneTile').length; j++) {
                ...
                $('.playerOneTile').eq(j).css("background-image", "url(" + player.rack[j].view + ")");
                
            }
        } else {
            for (j = 0; j < $('.playerTwoTile').length; j++) {
                $('.playerTwoTile').eq(j).text(player.rack[j].letter);
                $('.playerTwoTile').eq(j).css("background-image", "url(" + player.rack[j].view + ")");
    ...
    var turn = function() {
        ...
        if (players[0].rack.length === 0 || players[1].rack.length === 0) {
            ...
            $('.remainingTiles').text(shuffledBag.length+14);
    ...
    var showTiles = function() {
        if (isItPlayerOnesTurn()) {
            $('.playerOneBoxRow').show();
        } else {
            $('.playerTwoBoxRow').show();
        }
        $('.showTiles').hide();
    }
    ...
    $(document.body).on('drag', '.tileBox', function() {
    ...
    $(document.body).on('drop', '.box', function() {
        if (selected) {
            if (!($(this).hasClass('permInPlay')) && (!$(this).hasClass('tempInPlay'))) {
                $(this).text($('.selected').text());
                // $(this).css($('.selected').);
                $(this).css("background-image", $('.selected').css("background-image"));
                // let index = tileBag.findIndex(x => x.letter ===$('.selected').text());
                //   console.log(index);
                // $(this).css("background-image", "url('"+tileBag[index].view+"')");
